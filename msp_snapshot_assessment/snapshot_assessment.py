...
def cloud_account_sync_now(id):
    ...
    if mode == 'aws':
        url = f"https://api.dome9.com/v2/cloudaccounts/{id}/SyncNow"
    elif mode == 'azure':
        url = f"https://api.dome9.com/v2/AzureCloudAccount/{id}/SyncNow"
    elif mode == 'gcp':
        url = f"https://api.dome9.com/v2/GoogleCloudAccount/{id}/SyncNow"
    else:
...
def get_compliance_ruleset(rulesetid):
    ...
    url = f"https://api.dome9.com/v2/CompliancePolicy/{str(rulesetid)}"
    payload = {}
...
def remove_cloud_account(id):
    ...
    if mode == 'aws':
        url = f"https://api.dome9.com/v2/cloudaccounts/{id}"
    elif mode == 'azure':
        url = f"https://api.dome9.com/v2/AzureCloudAccount/{id}"
    elif mode == 'gcp':
        url = f"https://api.dome9.com/v2/GoogleCloudAccount/{id}"
    else:
        print(f'Invalid cloud provider mode: {mode}')
        return False
...
def unassociate_cc_policy(id):
    ...
    url = f"https://api.dome9.com/v2/Compliance/ContinuousCompliancePolicy/{id}"
    payload = {}
...
def delete_notification_policy(id):
    ...
    url = f"https://api.dome9.com/v2/Compliance/ContinuousComplianceNotification/{id}"
...
def get_scheduled_report_status(np_name, email):
    ...
    url = f'https://api.dome9.com/v2/Audit?&eventType=AssessmentScheduledReportSentEvent&eventsPerPage=100&fim=false&pageNum=1&startTimestamp={str(startdatetime.date())}T{str(startdatetime.time())}Z&endTimestamp={str(enddatetime.date())}T{str(enddatetime.time())}Z'
...
def process_account(account_added, extaccountid):
    ...
    notification_name = f'{OPTIONS.accountname}_snapshot_' + ''.join(
        random.choice(
    ...
    cronexpression = (
        f'0 {str(utc_datetime.minute)} {str(utc_datetime.hour)} 1/1 * ? *'
    )
...
# handle arguments
    ...
    if mode == 'aws':
        ...
        if account_added := add_aws_account(
            OPTIONS.accountname, OPTIONS.arn, OPTIONS.externalid
        ):
        ...
        if account_added := add_azure_account(
            OPTIONS.accountname,
        ...
        print(f'\nCloud Account \n-Name: {OPTIONS.accountname} \n-ID: {key["project_id"]}')
        if account_added := add_gcp_account(OPTIONS.accountname, key):
